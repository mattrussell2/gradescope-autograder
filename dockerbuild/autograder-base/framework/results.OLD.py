...
class GroupedResultsSubsets:
    ...
    def add(self, result):
        ...
        key = self.groupingFunction(result)
        #
        #  The result subset predicate will match on this particular key
        resultSubset = self.groups.get(key, 
                        ResultsSubset("GRS_{}".format(key),self.results_list, lambda r:self.groupingFunction(r) == key))
...
#            files, dict, dicts), file=sys.stderr)
        ...
        # byStudent is a GroupedResultsSubsets identifying the results for that student
        # byStudentData is for cross totals and other summaries such as
        # counts of failure reasons, etc.
        self.byStudent = GroupedResultsSubsets("ByStudent", self.resultsData, lambda r: r["student"]) # key is student login
        self.byStudentData = {}
        self.byTestset =  GroupedResultsSubsets("ByTestset", self.resultsData, lambda r: r["testset"]) # key is testset       
        self.bySuccess = GroupedResultsSubsets("BySuccess", self.resultsData, lambda r: r["success"])
        self.byTimedout = GroupedResultsSubsets("ByTimedout", self.resultsData, lambda r: r["timedout"])
...
#                  file=sys.stderr)
    ...
    def addByStudent(self, t, result):
        ...
        self.byStudent.add(result)           # add to set of results for this student
        ...
        self.byTestset.add(result)
        self.bySuccess.add(result)
