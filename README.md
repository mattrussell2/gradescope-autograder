...
```
Sometimes it may be desirable to have the file-to-be-written-to to preexist [i.e. the program would read from and then write to this file]. In this case, you can put these files into `copy`, then create a custom `testrunner.sh` file (see section of the same name) which copies the files from `/autograder/copy` over to `results/output` prior to running `autograde` -- one hiccup: you will need to run `autograde --dont-nuke /autograder/results/output`. The argument here is required because the default behavior will be to destroy this folder if it exists prior to running the autograder, which is useful behavior for testing, but will not matter in the usual case of one-off autograder runs for the students. 
