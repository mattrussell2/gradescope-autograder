...
class ResultsSubset:
    ...
    def add(self, r):
        debugit = False
        if self.debug_string == "GRS_build_executable":
            debugit = True
            print("ENTERING: ResultsSubset.add: ", r[INDEX_KEY])
        if self.predicate(r):
            if debugit:
                print("PREDICATE MATCHED, ADDING")
            self.m.add(r[INDEX_KEY])
        else:
           if debugit:
               print("PREDICATE NOT MATCHED")
        if debugit:
             print("LEAVING ResultsSubsset.ADD", repr(self))
...
class GroupedResultsSubsets:
    ...
    def get_key_comparitor(self, key):
        captured_key = key
        captured_singleton = singleton
        def key_is_in_group(r):
            return result = key in self.groupingFunction(r)
    ...
    def add(self, result):
        ...
        for key in keys:
            ...
            #  This makes a new one if it's the first time for this key
            resultSubset = self.groups.get(key, 
                                           ResultsSubset("GRS_{}".format(key),
                                                        self.results_list, 
                                                         self.get_key_comparitor(key)))
            if key == "build_executable":
                    print("\nABOUT TO CALL ADD: i={} keys[i]={} self.grouingFunction(r)={}".format(i, keys[i], self.groupingFunction(result)))
                    print("GRS about to add key: {} RS: {{ {} }}".format(key, repr(resultSubset)))
            resultSubset.add(result)
            self.groups[key] = resultSubset  # in case it's new
            if key == "build_executable":
                    print("FINISHED CALLING ADDi={} keys[i]={} self.grouingFunction(r)={}".format(i, keys[i], self.groupingFunction(result)))
