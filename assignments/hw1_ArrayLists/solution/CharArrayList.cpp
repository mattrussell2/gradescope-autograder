...
void CharArrayList::print() {
        std::cout << "[CharArrayList of size " << size() <<  " <<";
...
CharArrayList *CharArrayList::slice(int left, int right) {
        ...
        if((not (left >= 0 && left <= size()))
           or not (right >= 0 && right <= size())){
...
void CharArrayList::checkBoundsWithThrow(int index) {
...
void CharArrayList::checkEmptyWithThrow(std::string message) {
        if (isEmpty()){
